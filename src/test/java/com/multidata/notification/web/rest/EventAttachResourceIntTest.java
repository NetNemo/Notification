...
public class EventAttachResourceIntTest {
     ...
     */
    public static EventAttach createEntity(EntityManager em) {
    ...
    public void getEventAttach() throws Exception {
        ...
        restEventAttachMockMvc.perform(get("/api/event-attaches/{id}", eventAttach.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.id").value(eventAttach.getId().intValue()))
    ...
    public void updateEventAttach() throws Exception {
        ...
        // Disconnect from session so that the updates on updatedEventAttach are not directly saved in db
        em.detach(updatedEventAttach);
        updatedEventAttach
            .file(UPDATED_FILE)
    ...
    public void testEntityFromId() {
        assertThat(eventAttachMapper.fromId(42L).getId()).isEqualTo(42);
        assertThat(eventAttachMapper.fromId(null)).isNull();
    }
}
